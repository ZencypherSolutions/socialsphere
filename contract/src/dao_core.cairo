...
// This contract has the option to be connected with the governance contract by passing the
// governance contract address to the constructor of dao_core.
...
trait IDaoCore<TContractState> {
    ...
    // This function can only be called by the DAO Owner or sub-committee members.
    // For role argument each number represents a role, by default, a new member is assigned
    // asGeneral Member:
